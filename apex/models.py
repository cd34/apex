...
class AuthGroupUser(Base):
    ...
    user = relationship('AuthID', backref=backref('auth_groups', cascade='all, delete-orphan'))
    group = relationship('AuthGroup', backref=backref("auth_users", cascade='all, delete-orphan'))
...
::
    ...
    users = association_proxy('auth_users', "user")
...
::
    ...
    groups = association_proxy('auth_groups', 'group', creator=lambda group: AuthGroupUser(group=group))
...
::
    ...
    id = Column(types.Integer(), primary_key=True)
    auth_id = Column(types.Integer, ForeignKey(AuthID.id), index=True, nullable=False)
